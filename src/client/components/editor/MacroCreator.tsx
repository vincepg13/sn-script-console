...
import { createMacro, MacroData } from '@/lib/api';
...
export function MacroCreator({ open, setOpen }: MacroCreatorProps) {
  ...
  const createMutation = useMutation({
    mutationKey: ['macroCreate'],
    mutationFn: ({ macroData, signal }: { macroData: MacroData; signal: AbortSignal }) =>
      createMacro(macroData, signal),
    onSuccess: macro => {
      if (macro.widgetId) {
    ...
    onError: e => errorHandler(e, 'Failed to create macro widget'),
  });
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    createMutation.mutate({ macroData, signal: controllerRef.current.signal });
  };
